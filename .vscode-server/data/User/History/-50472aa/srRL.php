                    ...
                    @if ($oppasser)
                        ...
                        <div class="text-center mb-6">
                            ...
                            @if ($oppasser->profile_image)
                                <img src="{{ asset('storage/' . $oppasser->profile_image) }}" alt="Image of {{ $oppasser->naam }}" class="rounded-full mx-auto mb-4 max-w-[300px] max-h-[300px]object-cover">
                            @else
                            ...
                            @foreach ($oppassers as $oppasser)
                                @if (auth()->check() && ($oppasser->user_id !== auth()->id())) <!-- Skip the current user's oppasser -->
                                    <li class="mb-4 border-t border-white pt-4">
                                        <!-- Toon afbeelding van de oppasser -->
                                        @if ($oppasser->profile_image)
                                            <img src="{{ asset('storage/' . $oppasser->profile_image) }}" alt="Image of {{ $oppasser->naam }}" class="rounded-full mx-auto mb-4 max-w-[300px] max-h-[300px] w-full h-full object-cover">
                                        @else
                                            <p class="text-red-600">Geen profielfoto</p>
                                        @endif
                                        <strong>Naam:</strong> {{ $oppasser->naam }} <br>
