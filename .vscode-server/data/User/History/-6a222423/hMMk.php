    ...
    @foreach ($pets as $pet)
        <li class="mb-2">
            ...
            <form action="{{ route('pets.destroy', $pet->id) }}" method="POST" class="mt-2">
                @csrf
                @method('DELETE') <!-- This line specifies the request method -->
                <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md mb-4">Verwijderen</button> <!-- Added mb-4 here -->
            </form>
        </li>
    @endforeach
</ul>
