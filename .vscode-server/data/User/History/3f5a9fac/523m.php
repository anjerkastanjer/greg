                                        ...
                                        @auth
                                            @if ($pet->user->id !== auth()->user()->id) <!-- Only show button for pets not owned by the logged-in user -->
                                                <form action="{{ route('aanvragen.store', ['owner_id' => $pet->user->id]) }}" method="POST" class="mt-4">
                                                    ...
                                                    <button type="submit" class="class="mt-4 inline-flex items-center justify-center px-4 py-2 border border-black rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">">Meld je aan als Oppas</button>
                                                </form>
                                            @endif
                                        @endauth
